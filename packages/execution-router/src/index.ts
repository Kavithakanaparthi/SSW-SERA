...
export function buildExecutionRequest(input:{
 ...
 return assertContract("execution-request",r) as ExecutionRequest;
}
...
export function simulateEvmExecution(input:{request:ExecutionRequest;action:ActionContract;payloadBody:Record<string,unknown>;observedAt:string}):ExecutionResult{
 const req=assertContract("execution-request",input.request) as ExecutionRequest;const a=assertContract("action-contract",input.action);
 ...
 reason_codes:reasons,submission_ref:null,network_tx_id:null,observed_at:input.observedAt}) as ExecutionResult;
}
...
export async function submitExecution(input:{request:ExecutionRequest;transport:ChainSubmissionTransport;store:ExecutionReplayStore;now:string;submissionEnabled:boolean}):Promise<ExecutionResult>{
 const req=assertContract("execution-request",input.request) as ExecutionRequest;
 const block=(code:string):ExecutionResult=>assertContract("execution-result",{schema:"ssw.execution-result.v1",execution_request_id:req.execution_request_id,action_id:req.action_id,status:"BLOCKED",reason_codes:[code],submission_ref:null,network_tx_id:null,observed_at:input.now}) as ExecutionResult;
 ...
 try{
  ...
  return assertContract("execution-result",{schema:"ssw.execution-result.v1",execution_request_id:req.execution_request_id,action_id:req.action_id,status,
   reason_codes:[],submission_ref:out.submissionRef,network_tx_id:out.networkTxId??null,observed_at:input.now}) as ExecutionResult;
 }catch{
  return assertContract("execution-result",{schema:"ssw.execution-result.v1",execution_request_id:req.execution_request_id,action_id:req.action_id,status:"EXECUTION_STATUS_UNKNOWN",
   reason_codes:["TRANSPORT_OUTCOME_UNKNOWN"],submission_ref:null,network_tx_id:null,observed_at:input.now}) as ExecutionResult;
 }
}
