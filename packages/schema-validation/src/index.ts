...
const schemaFiles = [
  ...
  "ssw-holder-identity-context.v1.schema.json",
  "ssw-svid4ai-agent-context.v1.schema.json"
] as const;
...
function getIdentityPair(kind: ContractKind, value: unknown): { holderDid?: unknown; seraDid?: unknown } {
  ...
  if (kind === "voice-confidence-envelope" || kind === "monitoring-grant" || kind === "proactive-signal" || kind === "proactive-assessment" || kind === "route-request" || kind === "route-decision" || kind === "holder-identity-context" || kind === "svid4ai-agent-context") return {holderDid:v.holder_did,seraDid:v.sera_agent_did};
